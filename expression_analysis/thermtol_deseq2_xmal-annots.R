...
#samples <- samples[samples$temp == "33.5", ]

samples

files <- file.path(dir, "kallisto_posttrim_xmal-inferred-txtome", paste(samples$file_basename,"kallisto",sep="_"), "abundance.h5")
names(files) <- paste0(samples$sample)
files

## load transcript annotations
txdb <- makeTxDbFromGFF(file="input_files/refs/xiphophorus_malinche_14Nov2018_HyZ96_start-codons_formatted-for-txdb.gtf", format="gtf")
...
names(res_list)
#XXX
of_header   <- "TT-brain-33c-xmal-gtf_"
