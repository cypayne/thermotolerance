...
trans.both<-subset(geneset22c,subset=F1_transgress.high=="1"|F1_transgress.low=="1") #23
trans.both_hubgenes<-subset(subset(geneset22c,subset=F1_transgress.high=="1"|F1_transgress.low=="1"), Gene %in% hub.genes) #0
trans.low_hubgenes<-subset(subset(geneset22c,subset=F1_transgress.low=="1"), Gene %in% hub.genes) #0
trans.high_hubgenes<-subset(subset(geneset22c,subset=F1_transgress.high=="1"), Gene %in% hub.genes) #0
...
quantile(allGenes_withinTransgressive,probs=c(0.01,0.025,0.9,0.95,0.99))
#brain: 22C:Kwithin=600 - 90% quantile - 10, 95% - 12, 99% - 14. 15 (the number of hub genes that are transgressive) is greater than the 95% quantile so this is significant.
#liver: 22C:Kwithin=607 - 1% quantile - 0, 90%  - 2, 95% - 2, 99% - 3. 0 (the number of hub genes that are transgressive) is within the 95%-CI, not significant
...
hist(allGenes_withinTransgressive)
quantile(allGenes_withinTransgressive,probs=c(0.01,0.05,0.9,0.95,0.98))
#brain: 33C:Kwithin=600 - 1% quantile - 0, 5% - 0, 90% - 5, 95% - 6. 0 (the number of hub genes that are transgressive at 33c) is not significant.
#liver: 33C:Kwithin=600 - 1% quantile - 0, 90% - 3. 0 (the number of hub genes that are transgressive at 33c) is not significant.
...
#hub.genes<-hub.genes_Kwithin$X #607
hub.genes<-hub.genes_KTotal$X #639
header="TT-liver-WGCNA-onehot_"
...
# none for liver
#write.csv(hub.trans.both22c,paste0(header,"_all-kWithin-hub-expression-22c.csv"))
#write.csv(hub.trans.both33c,paste0(header,"_all-kWithin-hub-expression-33c.csv"))
