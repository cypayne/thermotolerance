...
for (x in 2:length(data[1,])){
  ...
  if(length(dat[,1]) >= (length(data[,1])*0.5) & length(unique(dat[,last_index]))>1){
    ...
    if(DEBUG) { cat(names[x],"\n",p_list,"\n") }
    results<-cbind(names[x],p_list[1],p_list[2],p_list[length(p_list)],(summary(model1)$coef[,"t value"])[length(p_list)],like_diff,length(dat[,1]))
    ...
    # Write out results
    if(track==0){
      write.table(results,file=out,append=TRUE,col.names=c("chrom.marker","intercept","mixture_prop","site","t-value","likelihood-diff","num_ind"),row.names=F,sep="\t",quote=FALSE)
      track=1
    } else{
      write.table(cbind(names[x],p_list[1],p_list[2],p_list[length(p_list)],(summary(model1)$coef[,"t value"])[length(p_list)],like_diff,length(dat[,1])),file=out,append=TRUE,col.names=FALSE,row.names=F,sep="\t",quote=FALSE)
